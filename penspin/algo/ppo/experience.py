...
class ExperienceBuffer(Dataset):
    def __init__(self, num_envs, horizon_length, batch_size, minibatch_size,
                 obs_dim, act_dim, priv_dim, critic_info_dim, point_cloud_sampled_dim, device, proprio_dim=32*30,proprio_length=30):
        self.device = device
        self.num_envs = num_envs
        self.transitions_per_env = horizon_length
        self.priv_info_dim = priv_dim
        self.critic_info_dim = critic_info_dim
        ...
        self.storage_dict = {
            ...
            'critic_info': torch.zeros((self.transitions_per_env, self.num_envs, self.critic_info_dim), dtype=torch.float32, device=self.device),
            ...
            'tactile_hist': torch.zeros((self.transitions_per_env, self.num_envs, 30, 32), dtype=torch.float32, device=self.device),
            'obj_ends': torch.zeros((self.transitions_per_env, self.num_envs, 3, 6), dtype=torch.float32, device=self.device),
    ...
    def __getitem__(self, idx):
        ...
        return input_dict['values'], input_dict['neglogpacs'], input_dict['advantages'], input_dict['mus'], \
            ...
            input_dict['obses'], input_dict['priv_info'], input_dict['critic_info'], \
            input_dict['point_cloud_info'], input_dict['proprio_hist'], \
            input_dict['tactile_hist'], input_dict['obj_ends']
