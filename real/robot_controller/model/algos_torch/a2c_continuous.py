...
class A2CAgent(a2c_common.ContinuousA2CBase):
    def __init__(self, base_name, params):
        ...
        if isinstance(obs_shape, dict):
            self.distill = True
            main_shape = {'obs': obs_shape['student_obs'], 'pointcloud': obs_shape['pointcloud']}  # obs_shape['student_obs']
            teacher_shape = obs_shape['obs']
            teacher_build_config = {
                'actions_num' : self.actions_num,
                'input_shape' : teacher_shape,
                'num_seqs' : self.num_actors * self.num_agents,
                'value_size': self.env_info.get('value_size',1),
                'normalize_value' : self.normalize_value,
                'normalize_input': self.normalize_input,
            }
            self.teacher = self.network.build(teacher_build_config)
            self.teacher.to(self.ppo_device)
            #self.restore("runs/fspc-no-stack-c/fspc-no-stack-cS1.0_C0.0_M0.02023-05-22_05-51-14-83810/nn/fspc-no-stack-c.pth", "teacher")
            self.restore("runs/fspc-stack4-c/fspc-stack4-cS1.0_C0.0_M0.02023-05-22_05-50-23-83810/nn/fspc-stack4-c.pth", "teacher")
        else:
            self.distill = False
