    ...
    def __init__(self, obs_shape, normalize_value, normalize_input, value_size):
        ...
        if normalize_input:
            if isinstance(obs_shape, dict):
                ...
                self.running_mean_std = RunningMeanStd(obs_shape['obs'])
            else:
    ...
    class Network(BaseModelNetwork):
        ...
        def forward(self, input_dict):
            ...
            if is_train:
                ...
                ent_action = ent_action.clip(-0.99, 0.99)
                ent_log_prob = distr.log_prob(ent_action).sum(-1, keepdim=True)
                entropy = - ent_log_prob
                # entropy = distr.entropy().sum(dim=-1)
                #prev_neglogp = self.neglogp(prev_actions, mu, sigma, logstd)
                # if prev_actions is not None:
                prev_actions = prev_actions.clip(-0.99, 0.99)
                prev_neglogp = - distr.log_prob(prev_actions).sum(dim=-1) #self.neglogp(prev_actions, mu, sigma, logstd)
                # else:
                #     prev_neglogp = None
                ...
                selected_action = selected_action.clip(-0.99, 0.99)
                # neglogp = self.neglogp(selected_action, mu, sigma, logstd)
                # if prev_actions is not None:
                neglogp = - distr.log_prob(selected_action).sum(dim=-1)  # self.neglogp(prev_actions, mu, sigma, logstd)
                # else:
                #     neglogp = None
...
class ModelA2CContinuousLogStd(BaseModel):
    ...
    class Network(BaseModelNetwork):
        ...
        def forward(self, input_dict):
            is_train = input_dict.get('is_train', True)
            prev_actions = input_dict.get('prev_actions', None)
            #print(input_dict['obs']['obs'].shape, input_dict['obs']['pointcloud'].shape)
            #input_dict['obs'] = self.norm_obs(input_dict['obs'])
            ...
            # print('recv', input_dict['obs'])
            mu, logstd, value, states = self.a2c_network(input_dict)
            sigma = torch.exp(logstd)
            distr = torch.distributions.Normal(mu, sigma)
            # print(mu, logstd, sigma, distr)
