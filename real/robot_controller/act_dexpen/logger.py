...
class Logger(object):
    ...
    def log(self, key, value, step, n=1):
        ...
        if key.startswith('eval'):
            self.sw.add_scalar(key, value, step)
        else:
            if key in self.log_dict.keys():
                if step in self.log_dict[key].keys():
                    self.log_dict[key][step] += value
                else:
                    self.log_dict[key][step] = value
            else:
                self.log_dict[key] = {}
    ...
    def dump(self, step, dataset_size):
        for key in self.log_dict.keys():
            self.sw.add_scalar(key, self.log_dict[key][step]/dataset_size, step)
